...
export function Header() {
  ...
  return <header className="site-header">
    ...
    <nav className="desktop-nav" aria-label="Main navigation">{nav.slice(1, 7).map(([label, href]) => <a href={href} key={href}>{label}</a>)}</nav>
    ...
    {open && <div className="mobile-menu">{nav.map(([label, href], index) => <a href={href} key={href}><span>0{index + 1}</span>{label}</a>)}</div>}
  </header>;
}
...
export function HomeExperience() {
  ...
  return <>
    ...
    <footer><div className="footer-top"><div className="footer-brand"><span className="brand-mark">DS</span><h3>Agro Tourism<br /><small>& Resort</small></h3><p>Escape the city. Experience nature. Live luxury.</p></div><div><p className="footer-label">Explore</p>{nav.slice(1, 7).map(([label, href]) => <a href={href} key={href}>{label}</a>)}</div><div><p className="footer-label">Connect</p><a href={`tel:+91${contact.phones[0]}`}>{contact.displayWhatsapp}</a><a href={contact.instagram} target="_blank" rel="noreferrer">Instagram ↗</a><a href={contact.maps} target="_blank" rel="noreferrer">Google Maps ↗</a></div></div><div className="footer-bottom"><span>© {new Date().getFullYear()} DS Agro Tourism & Resort</span><span>Facts, rates and availability confirmed directly by the resort.</span></div></footer>
